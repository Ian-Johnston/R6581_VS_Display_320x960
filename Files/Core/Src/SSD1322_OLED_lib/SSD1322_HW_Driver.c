...
extern volatile uint8_t SPI1_TX_completed_flag;
...
{
	HAL_GPIO_WritePin(GPIOA, OLED_CS_Pin, 0);
}
...
{
	HAL_GPIO_WritePin(GPIOA, OLED_CS_Pin, 1);
}
...
{
	HAL_GPIO_WritePin(GPIOA, OLED_DC_Pin, 0);
}
...
{
	HAL_GPIO_WritePin(GPIOA, OLED_DC_Pin, 1);
}
...
{
	HAL_GPIO_WritePin(GPIOA, OLED_RES_Pin, 0);
}
...
{
	HAL_GPIO_WritePin(GPIOA, OLED_RES_Pin, 1);
}
...
void SSD1322_HW_SPI_send_byte(uint8_t byte_to_transmit)
{
	...
	HAL_SPI_Transmit_DMA(&hspi1, &byte_to_transmit, 1);
	while(!SPI1_TX_completed_flag);
}
...
{
	SPI1_TX_completed_flag = 0;
	HAL_SPI_Transmit_DMA(&hspi1, array_to_transmit, array_size);
	while (!SPI1_TX_completed_flag);
}
